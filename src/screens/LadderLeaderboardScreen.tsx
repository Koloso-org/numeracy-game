...
import { LadderRank, getLadderTop, getMyLadderRank } from '../ladderLeaderboard';
import { isSupabaseConfigured } from '../supabase';

const LEVEL_LABEL: Record<LadderLevel, string> = { beginner: 'Beginner', expert: 'Expert' };

export default function LadderLeaderboardScreen({
  level,
  username,
  onBack,
}: {
  ...
  const [top, setTop] = useState<LadderRank[]>([]);
  const [me, setMe] = useState<LadderRank | null>(null);

  useEffect(() => {
    if (!isSupabaseConfigured) return;
    let active = true;
    (async () => {
      const [t, m] = await Promise.all([getLadderTop(level, 20), getMyLadderRank(level)]);
      if (!active) return;
      setTop(t);
      setMe(m);
      setLoading(false);
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>🏆 {LEVEL_LABEL[level]} Ladder</Text>
        <View style={{ width: 54 }} />
      </View>
      ...
      {!isSupabaseConfigured ? (
        ...
        <>
          <View style={styles.colHeader}>
            ...
            <Text style={[styles.colScore, styles.colHeadText]}>RUNGS</Text>
...
function Row({ entry, highlight }: { entry: LadderRank; highlight: boolean }) {
  return (
    <View style={[styles.row, highlight && styles.rowMe]}>
      ...
      <Text style={styles.score}>{entry.best_score}</Text>
      <Text style={styles.time}>{fmtTime(entry.best_time_ms)}</Text>
    </View>
  );
}
